...
@section('content')
	<div class="grid-head">
		...
		<div class="head-content row">
			...
			<div class="col-sm-8">
				<a href="{{route('admin.user.grid',array('session_id'=>session()->getId()))}}" class="btn action-button mr-auto"><i class="fas fa-arrow-left"></i>&#9; {{ __('Back') }}</a>
			</div>
		</div>
		<div style="height: 15px;"></div>
	</div>
...
@else
@endif
	<form action="{{ route('admin.user.create',array('session_id'=>session()->getId())) }}" method="POST" class="add-admin-user-form m-auto">
		@csrf
