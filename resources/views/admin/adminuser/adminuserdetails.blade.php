...
<div class="grid-container">
	...
	<div class="grid-head">
		...
		<div class="head-content row">
			...
			<div class="col-sm-8">
				<a href="{{route('admin.user.grid',array('session_id'=>session()->getId()))}}" class="btn action-button mr-2"><i class="fa fa-arrow-left" aria-hidden="true"></i>{{ __(' Back') }}</a>
				@if($userCount > 1)
				<a href="{{route('admin.user.delete',['user_id'=>request('user_id'),'session_id'=>session()->getId()])}}" onclick="return confirm('Are you sure?')" class="btn action-button mr-2"><i class="fa fa-trash" aria-hidden="true"></i>{{ __(' Delete User') }}</a>
				@endif
			</div>
		</div>
	</div>
	...
	<div class="details-container">
		<div class="row m-auto">
			...
			<div class="col-sm-12">
				<form action="{{route('admin.user.details',array('user_id'=> request('user_id'),'session_id'=>session()->getId()))}}" method="POST" class="add-admin-user-form m-auto">
					@csrf
