@extends('admin.index')
@section('title', __('Admin User Grid / ArtCam'))
@section('body-class', 'artcam-admin-user-grid')
@section('content')
<div class="grid-head">
	...
	<div class="head-content row">
		...
		<div class="col-sm-8">
			<a href="{{ route('admin.user.create',array('session_id'=>session()->getId())) }}" class="btn action-button mr-auto"><i class="fa fa-user-plus" aria-hidden="true"></i>{{ __(' Add New User') }}</a>
		</div>
	</div>
	<div style="height: 15px;"></div>
			...
			<table class="table adminUserGridTable">
				...
				<tbody>
					...
					<tr>
						...
						<td>{{$adminuser->is_active==1 ?'Active':'Inactive'}}</td>
						<td><a href="{{ route('admin.user.details',['session_id'=>session()->getId(), 'user_id'=>$adminuser->user_id]) }}">{{__('More')}}</a></td>
					</tr>
					@endforeach
				</tbody>
			</table>
