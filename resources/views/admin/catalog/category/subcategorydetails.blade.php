<form action="{{route('admin.subcategory.update',array('session_id'=>session()->getId()))}}" method="post" enctype="multipart/form-data">
	@csrf
	<input type="hidden" name="subCatID" value="{{$subCategory->entity_id}}">
	<div class="modal-body">
		...
		<div class="form-group row">
			<label for="inputName" class="col-sm-3 col-form-label text-right required">{{ __('Sub Category Name') }}</label>
			<div class="col-sm-9">
				<input type="text" class="form-control" name="subcatName" id="inputName" value="{{$subCategory->name}}" required>
			</div>
		</div>
		<div class="form-group row">
